...
from pose_control.deepgen_pose_controlnet_v3 import DeepGenPoseControlNetV3
...
def save_checkpoint(
    ...
    path = (
        output_dir
        / f"controlnet_v3_step_{step:06d}.pt"
    )
    ...
    torch.save(
        {
            "step": step,
            "identity": identity,
            "controlnet_v3": adapter.state_dict(),
...
def main():
    ...
    adapter = DeepGenPoseControlNetV3(
    ...
    print(
        "\nPose ControlNet V3 trainable params:",
        f"{trainable_params:,}",
    )
    ...
    for step in range(
        ...
        # --------------------------------------------------
        # Target Skeleton -> DeepGen VAE pose latent
        #
        # 对齐官方 SD3 ControlNet：
        # control image 进入与生成模型一致的 latent space。
        # --------------------------------------------------
        with torch.inference_mode():
            pose_pixels = (
                pose_map * 2.0 - 1.0
