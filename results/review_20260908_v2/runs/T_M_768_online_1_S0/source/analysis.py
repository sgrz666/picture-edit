...
def quality_report(root):
    scores=json.loads((root/'scores.json').read_text()) if (root/'scores.json').exists() else {}
    ...
    lines=['# 原生编辑效果复核','',
           ...
           '- 固定全局 CPU/CUDA 随机数与去噪随机数后，独立进程重复实验的输出 PNG 哈希一致。',
           '- 明确指定墨镜、帽子和服装有助于当前招手案例保持配饰；不能外推为普遍身份保持。',
...
def main():
    p=argparse.ArgumentParser();p.add_argument('command',choices=['select','capacity','quality']);p.add_argument('--root',type=Path,required=True)
    args=p.parse_args();print(json.dumps({'select':select,'capacity':capacity,'quality':quality_report}[args.command](args.root),ensure_ascii=False))
