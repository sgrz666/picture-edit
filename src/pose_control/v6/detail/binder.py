...
class DetailTokenBinder(nn.Module):
    """Add person/source, region-type, and anatomical-side bindings."""

    def __init__(self, token_dim: int, person_binder: PersonTokenBinder) -> None:
        super().__init__()
        if person_binder.slot_embedding.embedding_dim != token_dim:
            raise ValueError("person_binder token dimension must match token_dim")
        self.person_binder = person_binder
    ...
    def forward(
        ...
        person_binding = self.person_binder.binding_for(
            source_indices, region_valid.any(dim=-1)
        )
