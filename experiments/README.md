...
> 两轮实验的**文本报告、汇总 JSON、可视化 Demo 和非大文件运行记录**已同步到仓库 `results/`（`review_20260908` 初版 + `review_20260908_v2` 正式版，不含 `.pt`/`.npy` 大张量），无需本目录即可阅读结论。

## 目录结构

```
experiments/
├── review_20260908/                    # 第一轮早期实验（6 个原生质量运行，已被 v2 取代）
└── review_20260908_v2/                 # 正式复核实验根目录（由 --root 指定）
    ├── runs/                           # 每个运行 ID 一个子目录
    │   └── <RUN_ID>/
